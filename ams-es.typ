make a typst tamplate based on this image and on this code // Sizes used across the template.
...
#let ams-article(
  ...
  // Theorems.
  show figure.where(kind: "teorema"): set align(start)
  show figure.where(kind: "teorema"): it => block(spacing: 11.5pt, {
    strong({
      it.supplement
...
// The ASM template also provides a teorema function.
#let teorema(body, numbered: true) = figure(
  body,
  kind: "teorema",
  supplement: [Teorema],
...
//////////////////////// 
// Lema
#let lema(body, numbered: true) = figure(
  body,
  kind: "teorema",
  supplement: [Lema],
  numbering: if numbered { n => counter(heading).display() + [#n] }
...
// Corolario
#let corolario(body, numbered: true) = figure(
  body,
  kind: "teorema",
...
// Proposición
#let proposicion(body, numbered: true) = figure(
  body,
  kind: "teorema",
...
// Ejemplo
#let ejemplo(body, numbered: true) = figure(
  body,
  kind: "teorema",
  supplement: [Ejemplo],
  numbering: if numbered { n => counter(heading).display() + [#n] }
...
// Observación
#let observacion(body, numbered: true) = figure(
  body,
  kind: "teorema",
...
// Nota
#let nota(body, numbered: true) = figure(
  body,
  kind: "teorema",
  supplement: [Nota],
  numbering: if numbered { n => counter(heading).display() + [#n] }
...
// Definición
#let definicion(body, numbered: true) = figure(
  body,
  kind: "teorema",
...
// And a function for a prueba.
#let prueba(body) = block(spacing: 11.5pt, {
  emph[Prueba.]
  [ ]
  body
  h(1fr)

  // Add a word-joiner so that the prueba square and the last word before the
  // 1fr spacing are kept together.
  sym.wj

  // Add a non-breaking space to ensure a minimum amount of space between the
  // text and the prueba square.
  sym.space.nobreak
